...
class ProteinBertModel(nn.Module):
    ...
    def forward(self, tokens, repr_layers=[], need_head_weights=False, return_contacts=False):
        ...
        # Patrick: adapting these two lines for one-hot data
...
class MSATransformer(nn.Module):
    ...
    def forward(self, tokens, repr_layers=[], need_head_weights=False, return_contacts=False):
        ...
        batch_size, num_alignments, seqlen, alphabet_size = tokens.size()
        #assert tokens.ndim == 2
        
        # Patrick: adapting these two lines for one-hot data
        padding_mask = tokens[:,:,:,self.padding_idx].eq(1) 

        if not padding_mask.any():
            padding_mask = None

        x = self.embed_tokens(tokens)
        x += self.embed_positions(tokens.view(batch_size * num_alignments, seqlen, alphabet_size)).view(x.size())

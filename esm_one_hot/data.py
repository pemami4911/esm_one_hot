...
class Alphabet(object):
    ...
    def get_batch_converter(self):
        if self.use_msa:
            if self.use_one_hot:
                return OneHotMSABatchConverter(self)
            else:
                return MSABatchConverter(self)
        else:
            if self.use_one_hot:
                return OneHotBatchConverter(self)
            else:
    ...
    def from_architecture(cls, name: str) -> "Alphabet":
        ...
        return cls(standard_toks, prepend_toks, append_toks, prepend_bos, append_eos, use_msa, use_one_hot)
...
class OneHotBatchConverter(object):
    """Callable to convert an unprocessed (labels + strings) batch to a
    processed (labels + one-hot tensor) batch.
    """
    ...
    def __call__(self, raw_batch: Sequence[Tuple[str, str]]):
        ...
        # Initialize every one-hot encoding to padding idx = 1
        tokens[:,:,self.alphabet.padding_idx] = 1
        labels = []
        strs = []
        ...
        for i, (label, seq_str, seq_encoded) in enumerate(
            ...
            strs.append(seq_str)
            if self.alphabet.prepend_bos:
                tokens[i, 0, self.alphabet.padding_idx] = 0
                tokens[i, 0, self.alphabet.cls_idx] = 1
            # torch LongTensor of indices of tokens
            seq = torch.tensor(seq_encoded, dtype=torch.long)
            positions = torch.arange(start=int(self.alphabet.prepend_bos), end=int(self.alphabet.prepend_bos)+len(seq_encoded), dtype=torch.long)
            batch_idxs = i * torch.ones_like(positions[...,None], dtype=torch.long)
            padding_idxs = self.alphabet.padding_idx * torch.ones_like(positions[...,None], dtype=torch.long)

            tokens[batch_idxs, positions, padding_idxs] = 0
            tokens[batch_idxs, positions, seq] = 1

            if self.alphabet.append_eos:
                tokens[i, len(seq_encoded) + int(self.alphabet.prepend_bos), self.alphabet.padding_idx] = 0
                tokens[i, len(seq_encoded) + int(self.alphabet.prepend_bos), self.alphabet.eos_idx] = 1

        return labels, strs, tokens


class MSABatchConverter(OneHotBatchConverter):

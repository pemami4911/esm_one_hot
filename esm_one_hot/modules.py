...
class LearnedPositionalEmbedding(nn.Embedding):
    ...
    def forward(self, input: torch.Tensor):
        """Input is expected to be of size [bsz x seqlen x alphabet_size]."""
        if input.size(1) > self.max_positions:
            raise ValueError(
                f"Sequence length {input.size(1)} above maximum "
                f" sequence length of {self.max_positions}"
            )
        mask = (input[:,:, self.padding_idx].ne(1)).int()
...
class OneHotEmbedding(nn.Module):
    ...
    def forward(self, one_hots):
        """
        one_hots is a torch.FloatTensor of shape [batch_size, max_sequence_len, 33]
        """
        return one_hots @ self.weight